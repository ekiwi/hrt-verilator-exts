...
fn rebuild_with_tied_input(fsm: &FSM, tied_input: usize, value: bool) -> (FSM, Vec<ExprRef>) {
    ...
    for gate in fsm.get_gates() {
        ...
        rebuilt.add_gate(
            gate.gate_type,
            new_output.xor(!gate.output.sign()),
    ...
    for latch in fsm.get_latches() {
        let output = mapping[latch.output.index()]
            .unwrap_variable()
            .xor(!latch.output.sign());
        rebuilt.add_latch(
            remap_value(latch.input, &mapping),
            output,
            latch.reset_value,
        );
        *rebuilt.get_variable_label_mut(output.index()) =
            fsm.get_variable_label(latch.output.index()).clone();
    }
    for (output, label) in fsm.get_outputs() {
        let index = rebuilt.add_output(remap_value(*output, &mapping));
        *rebuilt.get_output_label_mut(index) = label.clone();
    }
    for (assertion, label) in fsm.get_asserts() {
        let index = rebuilt.add_assert(remap_value(*assertion, &mapping));
        *rebuilt.get_assert_label_mut(index) = label.clone();
    }
    for (assumption, label) in fsm.get_assumes() {
        let index = rebuilt.add_assume(remap_value(*assumption, &mapping));
        *rebuilt.get_assume_label_mut(index) = label.clone();
    }
    for (cover, label) in fsm.get_covers() {
        let index = rebuilt.add_cover(remap_value(*cover, &mapping));
...
fn remap_model_values(model: &mut NamedFsm, mapping: &[ExprRef]) {
        ...
        .chain(&mut model.outputs)
    {
        for bit in &mut signal.bits {
            bit.value = remap_value(bit.value, mapping);
        }
    }
    ...
    {
        property.value = remap_value(property.value, mapping);
    }
}

fn reorder_model(model: &mut NamedFsm) {
